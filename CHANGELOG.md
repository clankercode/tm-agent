...
- Startup suggestion: when the chat is empty and idle, a concise "Try: scenery sampler" (or "scenery around your route" if the map already has blocks) row appears above the composer. Clicking fills the composer with a map-aware starter prompt — 4–8 sample scenery islands, screenshot them, pick one, iterate or grow the style, then build the route on top; reuse existing macroblocks and create new ones for repeated creative elements. Nothing auto-sends. New/clear restores the row.
